...
`addAlertHistoryEntry` (append-only, newest-first — docs/14 §8). Realistic
...
- **Automatic, non-opt-in read-aloud** — a `role="status"
  ...
  announced inconsistently across screen readers — starting empty and
...
- **Asymmetric three-action stack** — "End the call now" (Filled,
  ...
  state, doesn't dismiss the screen — matches "the user may still need
