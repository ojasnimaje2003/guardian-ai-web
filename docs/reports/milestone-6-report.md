...
`gap-xs` (both 4px) but doesn't carry the token's name — normalized to
`gap-xs` for consistency. `rounded-full` (circular elements) and one
...
- **Not done, flagged honestly again:** actual visual/interactive
  ...
  worth a real click-through and resize pass before this is shown
