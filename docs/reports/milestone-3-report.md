...
and multi-step forms that `curl`/`next build` can't exercise. Verification
...
real interactive testing would have caught immediately. It's worth being
...
having the button silently do nothing.
...
- **Not done, and flagged honestly:** real click-through browser testing
  ...
  confidence is lower than in prior milestones' verification. It would
  benefit from a manual pass (or automated E2E coverage) before this is
  shown externally.
